...
jQuery.Callbacks = function( options ) {
	...
	// 通过字符串optionsCache寻找有没有相应的缓存，如果没有就创建一个，有则引用
	// 将字符串格式的参数转化为对象
	...
	var // Last fire value (for non-forgettable lists)
		...
		// options.once为假时，stack就是false
		stack = !options.once && [],
		// Fire callbacks
		// 内部函数，触发回调函数列表
		fire = function( data ) {
			// options.memory为真时，memory = data
			...
			if ( list ) {
				...
				// options.once为假时，stack就是false，这时候如果memory也是false，就要调用disable方法禁用回调函数列表的调用了
				else {
					self.disable();
				}
			}
		...
		self = {
				...
				if ( list ) {
					...
					(function add( args ) {
						jQuery.each( args, function( _, arg ) {
							...
							if ( type === "function" ) {
								// !options.unique为真说明可以重复；!self.has( arg )为真说明当前list不存在这个函数
								if ( !options.unique || !self.has( arg ) ) {
									list.push( arg );
								}
							} else if ( arg && arg.length && type !== "string" ) {
								//递归调用，当arg是数组时，继续遍历该数组
								add( arg );
							}
						});
					})( arguments );
					...
					jQuery.each( arguments, function( _, arg ) {
						...
						while( ( index = jQuery.inArray( arg, list, index ) ) > -1 ) {
							...
							if ( firing ) {
								...
								// 影响当前执行的Callbacks实例中的回调函数，函数删除后序号减1
			...
			// has方法其实存在两种判断：一种传入参数的判断，一种没有传入参数的判断
			has: function( fn ) {
...
jQuery.extend({
	...
	attr: function( elem, name, value ) {
		...
		// 节点类型是文本，注释，属性就直接返回不作任何操作
		if ( !elem || nType === 3 || nType === 8 || nType === 2 ) {
			return;
		}

		// Fallback to prop when attributes are not supported
		// 有些节点不支持attributes的操作，比如说document对象
		...
		if ( nType !== 1 || !jQuery.isXMLDoc( elem ) ) {
			...
			//attrHooks：jQuery中处理兼容性的一个东西，hooks不等于undefined说明存在兼容性问题
			hooks = jQuery.attrHooks[ name ] ||
				( jQuery.expr.match.bool.test( name ) ? boolHook : nodeHook );
			//jQuery.expr.match.bool匹配checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped
		}

		//说明是设置值
		if ( value !== undefined ) {
			//传入的属性值是null的时候其实是删除属性值
			if ( value === null ) {
	...
	attrHooks: {
		type: {
			//value指的是属性名
			set: function( elem, value ) {
				//先利用jQuery.support检测是否支持，然后解决兼容性问题
